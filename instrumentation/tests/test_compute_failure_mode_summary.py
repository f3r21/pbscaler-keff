...
def test_merge_summary_fills_zeros_and_attaches_metadata(tmp_path: Path):
    ...
    # Metrics added 2026-05-21 (literature alignment + convergence, Cap_3:246).
    p95 = pd.DataFrame([{"service": "checkout", "p95_latency_mean": 300.0}])
    p99 = pd.DataFrame([{"service": "checkout", "p99_latency_mean": 400.0}])
    cpu_int = pd.DataFrame([{"service": "adservice", "cpu_usage_integral_seconds": 120.0}])
    convergence = pd.DataFrame([{"service": "checkout", "convergence_time_s": 3.0,
                                 "convergence_events": 1}])
    metadata = {"benchmark": "online-boutique", "controller": "vanilla",
                "workload": "step", "rep": 2, "seed": 242}

    summary = merge_summary(phantom, double, ping_pong, replica, p90, slo,
                            p95, p99, cpu_int, convergence, metadata)
    ...
    # New metrics present and zero-filled where absent
    assert summary.loc["checkout", "p95_latency_mean"] == 300.0
    assert summary.loc["checkout", "cpu_usage_integral_seconds"] == 0
    assert summary.loc["adservice", "cpu_usage_integral_seconds"] == 120.0
    assert summary.loc["adservice", "p95_latency_mean"] == 0.0
    # metadata propagated
