...
- avg_replica_count, max_replica_count, replica_count_std: MEAN across
  services, then MEAN across reps.
- slo_violation_rate, p90/p95/p99_latency_mean: CHECKOUT-SCOPED — valor del
  servicio `checkoutservice` (endpoint principal del SLO, Cap_3:238), NO
  promedio entre servicios. Promediar diluye la señal con ~8 backends que
  responden en <10ms y nunca violan el umbral. Ver `aggregate_per_run`.
...
# Latencia/SLO se evalúan sobre el endpoint principal (checkout), NO promediadas
# entre servicios — Cap_3:238 ("la violación se evalúa sobre la P90 del endpoint
# principal", checkout de Online Boutique). Ver aggregate_per_run.
...
def aggregate_per_run(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Las métricas de latencia/SLO (`slo_violation_rate`, p90/p95/p99) son
    CHECKOUT-SCOPED: se toman del servicio ``checkoutservice``, no del promedio
    entre servicios. Cap_3:238 define la violación de SLO sobre la P90 del
    endpoint principal (checkout de Online Boutique); promediar entre los ~10
    servicios diluye la señal con backends que responden en <10ms y nunca violan
    el umbral. El resto (phantom, *_count, cpu_integral, réplicas) sí agrega
    entre servicios según ``SERVICE_AGGREGATION``.
    """
    keys = ["controller", "benchmark", "workload", "rep"]
    system_aggs = {
        m: SERVICE_AGGREGATION[m]
        for m in SERVICE_AGGREGATION
        if m in df.columns and m not in CHECKOUT_SCOPED
    }
    grouped = df.groupby(keys, dropna=False, as_index=False).agg(system_aggs)

    # Métricas checkout-scoped: valor de la fila checkoutservice por run.
    ck_metrics = [m for m in CHECKOUT_SCOPED if m in df.columns]
    if ck_metrics and "service" in df.columns:
        ck = df[df["service"] == CHECKOUT_SERVICE]
        ck_vals = ck.groupby(keys, dropna=False, as_index=False)[ck_metrics].first()
        grouped = grouped.merge(ck_vals, on=keys, how="left")
    return grouped
