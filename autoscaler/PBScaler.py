...
logging.basicConfig(
    # INFO (no DEBUG): conserva INIT/ANOMALY/SCALE/GA_OPT/PAGERANK/WASTE (todos info) y
    # corta el ruido DEBUG por-edge + de la librería schedule (el log salía ~39MB/run).
    # Override con env PBSCALER_LOGLEVEL=DEBUG para depurar.
    level=getattr(logging, os.environ.get('PBSCALER_LOGLEVEL', 'INFO').upper(), logging.INFO),
...
class PBScaler:
    ...
    def build_abnormal_subgraph(self, ab_calls):
        ...
        for ab_call in ab_calls:
            ...
            # window). Skip these edges rather than KeyError. parche del
            # fork — does not affect OB which has fully populated metrics.
    ...
    def _safe_anomaly_detect(self):
        ...
        parche del fork — see also pagerank ZeroDivisionError guard."""
        try:
            self.anomaly_detect()
        except Exception:
