...
class PrometheusClient:
    ...
    # execute prom_sql — con timeout + retry. El port-forward de Prometheus puede caerse
    # ("lost connection to pod"); sin timeout una request a una conexión muerta cuelga
    # indefinidamente (colgó collect_metrics en el smoke 2026-05-31). timeout=(connect,read).
    def execute_prom(self, prom_url, prom_sql):
        ...
        for _attempt in range(3):
            try:
                ...
                last_err = e
                time.sleep(2)
        raise RuntimeError('execute_prom falló tras 3 intentos (%s): %s' % (prom_url, last_err))
